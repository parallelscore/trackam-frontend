...
export const DeliveryProvider: React.FC<DeliveryProviderProps> = ({ children }) => {
    ...
    // Fetch deliveries with optional filtering and caching
    const fetchDeliveries = useCallback(async (filters?: {
        ...
        const cacheKey = filters
            ? `deliveries_${filters.status || 'all'}_${filters.search || ''}_${filters.page || 1}_${filters.limit || 10}`
            : 'deliveries_default';
        ...
        // Check cache first
        const cachedData = getFromCache(deliveriesCache.current, cacheKey);
        if (cachedData) {
            setDeliveries(cachedData.items);
            setTotalDeliveries(cachedData.total);
        ...
        try {
            ...
            const fetchPromise = async () => {
                if (USE_MOCK_SERVICE) {
                    ...
                    saveToCache(deliveriesCache, cacheKey, resultData);
